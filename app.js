...
teamOrder.forEach((teamName, teamIndex) => {
    if (!foundTeams.has(teamName)) {
        infoMessages.push(`ℹ️ ${teamName}: no input provided`);
    } else {
...
if (copyBtn) {
copyBtn.className = `px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${ isCopied  ? 'bg-green-600 text-white'  : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }`;
copyBtn.innerHTML = `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <rect width="14" height="14" x="8" y="8" rx="2" ry="2"></rect> <path d="M4 16c-1.1 0-2-.9-2 2-2h10c1.1 0 2 .9 2 2"></path> </svg> ${isCopied ? 'Copied!' : 'Copy to Clipboard'}`;
}
}
